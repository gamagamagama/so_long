...
/*   Updated: 2025/02/24 16:39:14 by matus            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	bb->pl = (new_x - (int)player->setup->data->vp_size_x / 2) + bb->of;
	bb->pr = (new_x + (int)player->setup->data->vp_size_x / 2) - bb->of;
	bb->pt = (new_y - (int)player->setup->data->vp_size_y / 2) + bb->of;
	bb->pb = (new_y + (int)player->setup->data->vp_size_y / 2) - bb->of;
}

void	set_bb_wl(t_wl_pl_bb *bb, t_cord *wall, int wid, int hig)
{
...
bool	check_movement(t_game *player, int new_x, int new_y)
{
	...
	while (wall != NULL)
	{
		...
		if (bb->pr > bb->wl && bb->pl < bb->wr && bb->pb > bb->wt
			&& bb->pt < bb->wb)
		{
			free(bb);
			return (false);
